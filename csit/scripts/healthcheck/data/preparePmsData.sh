...
curlString="curl -k -X PUT -sw %{http_code} -H accept:application/json -H Content-Type:application/json "$httpx://localhost:$policy_agent_port/v2/services" --data-binary @${SHELL_FOLDER}/testdata/v2/service.json"
res=$($curlString)
echo "$res"
expect="201"
checkRes
...
curlString="curl -k -X PUT -sw %{http_code} -H accept:application/json -H Content-Type:application/json $httpx://localhost:$policy_agent_port/v2/policy?policy_id=aa8feaa88d944d919ef0e83f2172a5000&policytype_id=1&ric_id=ric1&service_id=controlpanel&transient=false --data-binary @${SHELL_FOLDER}/testdata/policy-osc.json"
res=$($curlString)
echo "$res"
expect="201"
checkRes
...
echo "create policy aa8feaa88d944d919ef0e83f2172a5100 to ric2 with no-type and service ric-registration via policy agent:"
curlString="curl -k -X PUT -sw %{http_code} -H accept:application/json -H Content-Type:application/json $httpx://localhost:$policy_agent_port/v2/policy?policy_id=aa8feaa88d944d919ef0e83f2172a5100&ric_id=ric2&service_id=controlpanel&transient=false --data-binary @${SHELL_FOLDER}/testdata/policy.json"
res=$($curlString)
echo "$res"
expect="201"
checkRes
...
curlString="curl -s -o /dev/null -I -w %{http_code} $httpx://localhost:$policy_agent_port/v2/policy?policy_id=aa8feaa88d944d919ef0e83f2172a5000"
res=$($curlString)
echo "$res"
expect="200"
checkRes
...
curlString="curl -s -o /dev/null -I -w %{http_code} $httpx://localhost:$policy_agent_port/v2/policy?policy_id=aa8feaa88d944d919ef0e83f2172a5100"
res=$($curlString)
echo "$res"
expect="200"
checkRes
